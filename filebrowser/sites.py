...
from filebrowser.settings import (ADMIN_THUMBNAIL, ADMIN_VERSIONS,
								  ...
								  UPLOAD_TEMPDIR, VERSIONS, VERSIONS_BASEDIR, VERSION_QUALITY)
...
def handle_file_upload(path, file, site):
	...
	try:
		...
		if ext in EXTENSIONS['Image']:
			print('Это картинка, делаем из неё .webp')
			tmpfile = File(tempfile.NamedTemporaryFile())
			file.name = root + '.webp'
			file_path = os.path.join(path, file.name)
			img = Image.open(file)
			img.save(tmpfile, format=Image.EXTENSION['.webp'], quality=VERSION_QUALITY, optimize=True)
			uploadedfile = site.storage.save(file_path, tmpfile)
		else:
			file_path = os.path.join(path, file.name)
